...
public class LifecycleTransformer<T>
  ...
  LifecycleTransformer(Observable<ActivityLifecycle> lifecycleObservable) {
    ...
    mObservable =
        Observable.combineLatest(observable.take(1).map(ACTIVITY_LIFECYCLE), observable.skip(1),
            new BiFunction<ActivityLifecycle, ActivityLifecycle, Boolean>() {
              @Override public Boolean apply(@NonNull ActivityLifecycle ActivityLifecycle,
                  @NonNull ActivityLifecycle ActivityLifecycle2) throws Exception {
  ...
  LifecycleTransformer(Observable<ActivityLifecycle> lifecycleObservable,
    ...
    mObservable = lifecycleObservable.filter(new Predicate<ActivityLifecycle>() {
      @Override public boolean test(@NonNull ActivityLifecycle event) throws Exception {
  ...
  @Override public String toString() {
    return "LifecycleTransformer{" + "mObservable=" + mObservable + '}';
  }
  ...
  private static final Function<ActivityLifecycle, ActivityLifecycle> ACTIVITY_LIFECYCLE =
      new Function<ActivityLifecycle, ActivityLifecycle>() {
        @Override public ActivityLifecycle apply(@NonNull ActivityLifecycle lastEvent)
            throws Exception {
          switch (lastEvent) {
            case onCreate:
            ...
            case onDestroy:
              throw new IllegalStateException(
                  "Cannot injectRxLifecycle to Activity lifecycle when outside of it.");
            default:
